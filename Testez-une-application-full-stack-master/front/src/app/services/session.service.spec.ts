...
describe('SessionService', () => {
  ...
  it('should add a participant to a session', () => {
    ...
    let addedParticipant: any; // Créer une variable pour stocker la réponse de la requête HTTP

    serviceApi.participate(sessionId, userId).subscribe((response: any) => {
      addedParticipant = response; // Stocker la réponse de la requête HTTP
    });
    ...
    req.flush(mockResponse); // Simuler la réponse de la requête HTTP

    // Vérifier que la réponse de la requête contient les données attendues du participant ajouté
